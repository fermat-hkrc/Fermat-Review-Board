...
`CallManagerService` 中多个方法（`GetProxyObjectPtr`、`MakeCall`、`GetCallState`、`HasCall` 等）未遵循同类方法的标准双步权限验证模式，缺少 `CheckCallerIsSystemApp()` 或 `CheckPermission()` 调用，导致未授权进程可获取蓝牙通话代理对象、触发任意号码拨号或查询用户通话隐私状态。
## 背景
...
- **提单**: [telephony_call_manager#1913](https://gitcode.com/openharmony/telephony_call_manager/issues/1913)
- **修复 PR**: 关联 PR "修改call_manager_service部分函数权限校验问题"（yuanxinru，2026-05-30 前后）
- **上游状态**: 已关闭（已完成）

**问题1（GetProxyObjectPtr）**: 已修复，关联 PR 合并。

**问题2-5（MakeCall / GetCallState / IsEmergencyPhoneNumber / FormatPhoneNumber）**: 开发者明确回复无需修改：

---

## 开发者回复（部分拒绝）

> **yuanxinru** — 2026-06-01
>
> 问题2：MakeCall() — 无需修改：innerapi，不需要校验
>
> 问题3：GetCallState() / HasCall() — 无需修改：public接口，无需权限
>
> 问题4：IsEmergencyPhoneNumber() — 无需修改：public接口，无需权限
>
> 问题5：FormatPhoneNumber() / FormatPhoneNumberToE164() — 无需修改：public接口，无需权限
