...
`customization_config_policy` 的核心逻辑在单个 C 文件（`config_policy_utils.c`, 549 行）中。该文件使用 securec 函数（`snprintf_s`, `strcpy_s` 等）防止缓冲区溢出，因此不存在 ASan 可检测的内存破坏。但路径遍历漏洞可以通过 Target-Compile 直接验证——编译真实代码，调用真实函数，观察路径构造结果。
...
此漏洞是信息泄露（CWE-22），不会导致 ASan 可检测的内存错误。代码使用 `snprintf_s` 防止了缓冲区溢出。PoC 证明的是**漏洞代码路径通过真实编译的 .a 正确执行**，并且路径遍历在模拟环境下成功。

**7.2 测试环境 vs 真实设备**

| 维度 | 测试环境 (Ubuntu) | 真实设备 (OHOS) |
|------|-------------------|-----------------|
| `/system` 目录 | 不存在 | 存在 |
| `access()` 结果 | -1 (ENOENT) | 0 (成功) |
| 函数返回值 | NULL | 遍历后的路径 |
| JS API | 不可用 | `getOneCfgFile("../../etc/passwd")` 可调用 |

在测试环境中，`GetOneCfgFile` 返回 NULL 因为配置目录不存在。但这不影响漏洞判断——代码路径已通过真实编译验证，路径构造逻辑正确执行。

**7.3 漏洞的危害评估**

路径遍历允许探测文件存在性，但不能读取文件内容。单独使用时危害有限（MEDIUM），但：
- 可用于信息收集（探测特定应用是否安装、特定配置文件是否存在）
- 可与其他漏洞链式利用
- JS API 的暴露面很广，任何应用无需权限即可调用
